...
public class ProfileFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        Button organizationDashboardButton = view.findViewById(R.id.organizationDashboardButton);


        ImageView verificationBadge = view.findViewById(R.id.verificationBadge);


        // Set a click listener to the Button
        organizationDashboardButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Handle Button click here
                // Start OrganizationDashboardActivity
                startActivity(new Intent(getActivity(), DashboardGraphActivity.class));
            }
        });
        ...
        userRef.child("verified").addValueEventListener(new ValueEventListener() {
            @Override
        ...
        /*By using orderByChild query we will show the detail from a
        node whose key named email has value equal to currently signed in email.
        it will search all nodes, where the key matches it will get its detail*/
                DatabaseReference userTypeRef = firebaseDatabase.getReference("Users").child(user.getUid()).child("usertype");
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        for (DataSnapshot ds : dataSnapshot.getChildren()) {
                                ...
                                public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                    if (dataSnapshot.exists()) {
                                        try {
                                            ...
                                            if (userData != null) {
                                                // Retrieve the "userType" field from the HashMap
                                                String userType = (String) userData.get("userType");

                                                if ("Organization".equals(userType)) {
                                                    organizationDashboardButton.setVisibility(View.VISIBLE);
                                                } else {
                                                    organizationDashboardButton.setVisibility(View.GONE);
                                                }
                                            } else {
                                                // Handle the case where 'userData' is null
                                            }
                                        } catch (Exception e) {
                                            Log.e("Firebase Conversion", "Failed to convert userType to String.", e);
                                        }
                                    }
                                }
                            ...
                            //if there is any exception while getting image then set default
                                ...
                                Picasso.get().load(R.drawable.ic_profile_image).into(avatarIv);
        ...
        /*parameter "key" will contain value:
          ...
          or     "phone" which is key in user's database which is used to update user's phone*/
            ...
            private void showImagePicDialog() {
                //show dialog containig options Camera and gallery to pick the image
            ...
            public void onActivityResult(int requestCode, int resultCode, Intent data) {
                ...
                if (resultCode == getActivity().RESULT_OK) {
                    ...
                    if (requestCode == IMAGE_PICK_GALLERY_CODE) {
                        ...
                        Uri image_uri = data.getData(); // Example: data.getData() gets the selected image URI
            ...
            private void uploadProfileCoverPhoto(final Uri uri) {
                ...
                //e.g. Users_Profile_cover_Imgs/image_photo1.jps
                //e.g. Users_Profile_cover_Imgs/image_photo2.jps
                ...
                storageReference2nd.putFile(uri)
                        .addOnSuccessListener(new OnSuccessListener<UploadTask.TaskSnapshot>() {
                            ...
                            /*First Parameter is profileOrCoverPhoto that has value "image" or "Cover"
                             which are keys in user's database where url of image will be saved in
                              one of them
...
//            user not signed in, go to main activity
    ...
    private void checkAndSetVerificationStatus() {
        ...
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot ds : dataSnapshot.getChildren()) {
                    // Get the values for likes, comments, and shares from each post
                    int likes = ds.child("like").getValue(Integer.class);
                    int comments = ds.child("comment").getValue(Integer.class);
                    int shares = ds.child("share").getValue(Integer.class);

                    // Calculate the total value for this user, considering the weights
                    totalValue += (likes * 5) + (comments * 8) + (shares * 10);
                }

                // Check if the total value meets the verification criteria
                String verificationStatus = (totalValue >= 100000) ? "verified" : "notverified";

                // Update the user's verification status in the database
                DatabaseReference userRef = FirebaseDatabase.getInstance().getReference("Users").child(uid);
                userRef.child("verified").setValue(verificationStatus);
            }
...
//                menu.findItem(R.id.org_action_add_post).setVisible(true);
            ...
            public boolean onOptionsItemSelected(@NonNull MenuItem item) {
                ...
                if (id == R.id.org_action_add_post) {
                    startActivity(new Intent(getActivity(), OrgAddPostActivity.class));
