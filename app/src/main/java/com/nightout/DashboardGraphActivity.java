...
//        com.nightout.integration.initializePython((Context) integration); // Pass the activity's context
    ...
    private void calculateStatisticsFromDatabase() {
        String currentUserUid = FirebaseAuth.getInstance().getCurrentUser().getUid();
        FirebaseDatabase database = FirebaseDatabase.getInstance();

        // Reference to "DataCost" node
        DatabaseReference dataRef = database.getReference("DataCost");

        // Create a query to filter data by the "uid" field
        Query query = dataRef.orderByChild("uid").equalTo(currentUserUid);
        ...
        query.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                // Now you can call your calculation methods here
                calculateMostProfitableMonth(dataList);
                calculateHighestTicketCountInSingleMonth(dataList);
                calculateMostProfitableCategory(dataList);
                calculateCategoryWithMostSponsors(dataList);
                calculateCategoryWithMostTicketCount(dataList);
            }
...
// Define your calculation methods outside of the calculateStatisticsFromDatabase method

// Define your calculation methods outside of the calculateStatisticsFromDatabase method

    // Method to calculate the most profitable month
    private void calculateMostProfitableMonth(List<ModelDashboard> dataList) {
        double maxProfit = Double.MIN_VALUE;
        String mostProfitableMonth = "";

        for (ModelDashboard data : dataList) {
            double totalProfit = data.getTotalProfit();
            if (totalProfit > maxProfit) {
                maxProfit = totalProfit;
                mostProfitableMonth = data.getMonth(); // Assuming you have a method to get the month
            }
        }

        // Now, 'mostProfitableMonth' contains the most profitable month
        // You can display or use this information as needed
    }


    // Method to calculate the highest ticket count in a single month
    private void calculateHighestTicketCountInSingleMonth(List<ModelDashboard> dataList) {
        int maxTicketCount = 0;
        String monthWithHighestTicketCount = "";

        for (ModelDashboard data : dataList) {
            int ticketCount = Integer.parseInt(data.getTicketCount());
            if (ticketCount > maxTicketCount) {
                maxTicketCount = ticketCount;
                monthWithHighestTicketCount = data.getMonth(); // Assuming you have a method to get the month
            }
        }

        // Now, 'monthWithHighestTicketCount' contains the month with the highest ticket count
        // You can display or use this information as needed
    }

    // Method to calculate the category that is the most profitable
    private void calculateMostProfitableCategory(List<ModelDashboard> dataList) {
        // Create a map to store the total profit for each category
        Map<String, Double> categoryProfitMap = new HashMap<>();

        for (ModelDashboard data : dataList) {
            String category = data.getCategory(); // Replace with your method to get the category
            double totalProfit = data.getTotalProfit();

            // Update the total profit for the category
            categoryProfitMap.put(category, categoryProfitMap.getOrDefault(category, 0.0) + totalProfit);
        }

        // Find the category with the highest total profit
        String mostProfitableCategory = Collections.max(categoryProfitMap.entrySet(), Map.Entry.comparingByValue()).getKey();

        // Now, 'mostProfitableCategory' contains the most profitable category
        // You can display or use this information as needed
    }

    // Method to calculate the category with the most sponsors
    private void calculateCategoryWithMostSponsors(List<ModelDashboard> dataList) {
        // Create a map to store the count of sponsors for each category
        Map<String, Integer> categorySponsorCountMap = new HashMap<>();

        for (ModelDashboard data : dataList) {
            String category = data.getCategory(); // Replace with your method to get the category
            int sponsorIncome = Integer.parseInt(data.getSponsorIncome()); // Assuming you have a method to get the sponsor count

            // Update the sponsor count for the category
            categorySponsorCountMap.put(category, categorySponsorCountMap.getOrDefault(category, 0) + sponsorIncome);
        }

        // Find the category with the highest sponsor count
        String categoryWithMostSponsors = Collections.max(categorySponsorCountMap.entrySet(), Map.Entry.comparingByValue()).getKey();

        // Now, 'categoryWithMostSponsors' contains the category with the most sponsors
        // You can display or use this information as needed
    }


    // Method to calculate the category with the most ticket count
    private void calculateCategoryWithMostTicketCount(List<ModelDashboard> dataList) {
        // Create a map to store the total ticket count for each category
        Map<String, Integer> categoryTicketCountMap = new HashMap<>();

        for (ModelDashboard data : dataList) {
            String category = data.getCategory(); // Replace with your method to get the category
            int ticketCount = Integer.parseInt(data.getTicketCount());

            // Update the total ticket count for the category
            categoryTicketCountMap.put(category, categoryTicketCountMap.getOrDefault(category, 0) + ticketCount);
        }

        // Find the category with the highest total ticket count
        String categoryWithMostTicketCount = Collections.max(categoryTicketCountMap.entrySet(), Map.Entry.comparingByValue()).getKey();

        // Now, 'categoryWithMostTicketCount' contains the category with the most ticket count
        // You can display or use this information as needed
    }
